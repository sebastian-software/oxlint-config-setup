...
function renderCatalog(): string {
  ...
  lines.push(
    ...
    "## Stability boundary",
    "",
    "`react/react-compiler` is emitted only by the explicitly named experimental artifact and begins as a warning. Testing Library, Playwright, and Storybook are package-owned file-scoped integrations rather than ledger profiles; Testing Library inherits the upstream DOM or React preset, while Playwright and Storybook inherit flat/recommended. Every TypeScript loader automatically adds the 13 syntax-only predecessor SonarJS rules at the root; AI adds six further predecessor guardrails. Type-aware SonarJS rules remain excluded. The regular-expression plugin domain remains Deferred with an explicit re-entry gate in the migration guide.",
    "",
  );
  return lines.join("\n");
}
