...
The standard command invokes Oxlint directly. JavaScript, TypeScript, imports, React, JSX accessibility, Node.js, Vitest, and Jest are covered through native Oxlint plugins. Package-owned Testing Library and Playwright compatibility plugins apply only to canonical test and E2E files. Type-aware TypeScript rules use the pinned `oxlint-tsgolint` backend through the same command.
